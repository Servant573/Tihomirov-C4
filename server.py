...
def show_or_modify_task(uid):
    if bottle.request.method == "GET":
        return task_db[uid].to_dict()
    elif bottle.request.method == "PUT":
        if "description" in bottle.request.json:
            task_db[uid].description = bottle.request.json['description']
        if "is_completed" in bottle.request.json:
            task_db[uid].is_completed = bottle.request.json['is_completed']
